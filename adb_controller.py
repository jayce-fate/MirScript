...
# # 图片匹配,直到匹配成功或者超时
# def wait_till_match_any(template_paths,thresholds,max_time,step_time,scope = None):
...
# 				settings.screenshot_path,template_paths[index],thresholds[index],scope = scope)
# 			if(match_loc != None):
# 				return match_loc
...
# # 图片匹配，匹配成功后执行点击
# def wait_to_match_and_click(template_paths,thresholds,max_time,step_time
# 							,click_offset = None,scope = None):
# 	re = wait_till_match_any(template_paths,thresholds,max_time,step_time,scope = scope)
# 	if(re == None):
...
# def wait_while_match(template_paths,thresholds,max_time,step_time,scope = None):
...
# 			match_loc = image_processor.match_template(
# 				settings.screenshot_path,template_paths[i],thresholds[i],scope = scope)
# 			if(match_loc != None):
# 				break
